...
CHAT_GROUP = int(environ.get("CHAT_GROUP", "-1002209475737"))
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002209475737') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001819373091') # request channel id ( make sure bot is admin )
...
AI_LOGS = int(environ.get("AI_LOGS", "-1001819373091")) #GIVE YOUR NEW LOG CHANNEL ID TO STORE MESSAGES THAT THEY SEARCH IN BOT PM.... [ i have added this to keep an eye on the users message, to avoid misuse of Bot ]

#Auto approve 
CHAT_ID = [int(app_chat_id) if id_pattern.search(app_chat_id) else app_chat_id for app_chat_id in environ.get('CHAT_ID', '-1002209475737').split()]
...
OWNER_USERNAME = environ.get('OWNER_USERNAME', 'Isha_dalvi_x') # owner username without @

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001819373091')) #Log channel id ( make sure bot is admin )
DUMP_CHNL = int(environ.get('DUMP_CHNL', '-1001819373091'))
...
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1001819373091'))
